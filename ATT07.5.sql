...
BEGIN
    ...
    DECLARE @vMinAtual INT; 
    DECLARE @vMaxAtual INT;

    WHILE @i < @numIntervalos -- Roda enquanto
    BEGIN 
        -- Cálculo dos limites da faixa atual
        SET @vMinAtual = CAST(@minSal + (@i * @amplitude) AS INT); 
        SET @vMaxAtual = CAST(@minSal + ((@i + 1) * @amplitude) AS INT); 
        
        -- Ajuste para não sobrepor valores
        ...
        SELECT @vMinAtual, @vMaxAtual, COUNT(*) 
        FROM dbo.instructor 
        WHERE salary BETWEEN @vMinAtual AND @vMaxAtual; 

        SET @i = @i + 1; -- Soma 1 ao contador @i pra que o código entenda que deve passar para a próxima faixa 
    END 

    -- 6. Exibir o resultado final ordenado de min para max
    SELECT * FROM @HistogramaTable ORDER BY valorMinimo; 
END;
GO
